...
#include <Arduino.h>
#include "board_config.h"  // MAX_ACTUATORS (4 or 5 depending on board)
...
enum class SyncCommandType : uint8_t {
    ...
    BUZZ,
    DEACTIVATE,
    PING,             // Clock sync request + keepalive (PRIMARY -> SECONDARY) - runs every 1 second
    PONG,             // Clock sync response + keepalive ack (SECONDARY -> PRIMARY)
    DEBUG_FLASH,      // Debug LED flash sync (PRIMARY -> SECONDARY)
    MACROCYCLE,       // Batch of buzz events for entire macrocycle (PRIMARY -> SECONDARY)
    MACROCYCLE_ACK    // Macrocycle acknowledgment (SECONDARY -> PRIMARY)
};
...
inline const char* syncCommandTypeToString(SyncCommandType type) {
    switch (type) {
        ...
        case SyncCommandType::MACROCYCLE: return "MACROCYCLE";
        case SyncCommandType::MACROCYCLE_ACK: return "MACROCYCLE_ACK";
        default: return "UNKNOWN";
    }
}
...
struct TherapyConfig {
    ...
    uint8_t numFingers;         // Number of fingers to use (1-MAX_ACTUATORS)
    ...
    TherapyConfig() :
        ...
        numFingers(MAX_ACTUATORS),  // All of the board's actuators by default
...
/**
 * @brief Maximum events in a macrocycle (3 patterns x MAX_ACTUATORS fingers)
 */
constexpr uint8_t MACROCYCLE_MAX_EVENTS = 3 * MAX_ACTUATORS;

/**
 * @brief Frequency encoding base (freqOffset = (freq - FREQ_BASE) / FREQ_STEP)
 */
constexpr uint16_t MACROCYCLE_FREQ_BASE = 200;
constexpr uint8_t MACROCYCLE_FREQ_STEP = 5;

/**
 * @brief Single buzz event within a macrocycle (packed for BLE transmission)
 *
 * Binary format: 6 bytes per event for compact BLE payload.
 * All times are relative to the macrocycle baseTime.
 *
 * Note: 'finger' is the SECONDARY finger (transmitted over BLE).
 * 'primaryFinger' is for local PRIMARY use only (NOT serialized).
 * In mirrored mode, primaryFinger == finger. In non-mirrored mode, they differ.
 */
struct __attribute__((packed)) MacrocycleEvent {
    uint16_t deltaTimeMs;   // Offset from baseTime in milliseconds (0-65535)
    uint8_t  finger;        // SECONDARY motor index (0 to MAX_ACTUATORS-1) - transmitted over BLE
    uint8_t  amplitude;     // Intensity percentage (0-100)
    uint16_t durationMs;    // ON duration in milliseconds (supports up to 65535ms)
    uint8_t  freqOffset;    // Encoded frequency: (freq - 200) / 5, supports 200-455 Hz
    uint8_t  primaryFinger; // PRIMARY motor index (0 to MAX_ACTUATORS-1) - local use only, NOT serialized

    MacrocycleEvent() : deltaTimeMs(0), finger(0), amplitude(0), durationMs(0), freqOffset(0), primaryFinger(0) {}

    MacrocycleEvent(uint16_t delta, uint8_t secFinger, uint8_t primFinger, uint8_t amp, uint16_t dur, uint16_t freqHz)
        : deltaTimeMs(delta)
        , finger(secFinger)
        , amplitude(amp)
        , durationMs(dur)
...
/**
 * @brief Complete macrocycle containing all buzz events
 *
 * Generated by PRIMARY and sent to SECONDARY as a single batched message.
 * Contains MACROCYCLE_MAX_EVENTS events (3 patterns x MAX_ACTUATORS fingers) with timing relative to baseTime.
 */
struct Macrocycle {
    uint32_t sequenceId;                            // Sequence number for ACK matching
    uint64_t baseTime;                              // Absolute activation time of event 0 (PRIMARY clock, µs)
    int64_t  clockOffset;                           // PTP clock offset for SECONDARY (µs)
    uint16_t durationMs;                            // Common duration for all events (from profile, supports up to 65535ms)
    uint8_t  eventCount;                            // Number of valid events (typically 12)
    MacrocycleEvent events[MACROCYCLE_MAX_EVENTS];  // Buzz events

    Macrocycle() : sequenceId(0), baseTime(0), clockOffset(0), durationMs(100), eventCount(0) {}

    /**
     * @brief Add an event to the macrocycle
     * @param deltaMs Offset from baseTime in milliseconds
     * @param secFinger SECONDARY motor index (transmitted over BLE)
     * @param primFinger PRIMARY motor index (local use only)
     * @param amp Amplitude percentage (0-100)
     * @param durMs Duration in milliseconds
     * @param freqHz Frequency in Hz
     * @return true if added, false if full
     */
    bool addEvent(uint16_t deltaMs, uint8_t secFinger, uint8_t primFinger, uint8_t amp, uint16_t durMs, uint16_t freqHz) {
        if (eventCount >= MACROCYCLE_MAX_EVENTS) return false;
        events[eventCount++] = MacrocycleEvent(deltaMs, secFinger, primFinger, amp, durMs, freqHz);
        return true;
    }

    /**
     * @brief Get total duration of macrocycle in milliseconds
     */
    uint16_t getTotalDurationMs() const {
        if (eventCount == 0) return 0;
        // Last event's delta + its duration
        return events[eventCount - 1].deltaTimeMs + events[eventCount - 1].durationMs;
    }
};
