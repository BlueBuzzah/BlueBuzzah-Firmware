...
/**
 * @brief Maximum events in a macrocycle (3 patterns x 4 fingers)
 */
constexpr uint8_t MACROCYCLE_MAX_EVENTS = 12;
...
struct __attribute__((packed)) MacrocycleEvent {
    ...
    uint8_t  finger;        // SECONDARY motor index (0-3) - transmitted over BLE
    ...
    uint8_t  primaryFinger; // PRIMARY motor index (0-3) - local use only, NOT serialized
 ...
 * Contains 12 events (3 patterns x 4 fingers) with timing relative to baseTime.
 */
struct Macrocycle {
