...
void test_TherapyEngine_startSession(void) {
    ...
    engine.startSession(7200s, PatternType::RNDP, 100.0f, 67.0f, 23.5f, 4, true);

    TEST_ASSERT_TRUE(engine.isRunning());
    TEST_ASSERT_FALSE(engine.isPaused());
    TEST_ASSERT_EQUAL(7200s, engine.getDurationSeconds());
}
...
void test_TherapyEngine_resets_stats_on_start(void) {
    TherapyEngine engine;
    engine.startSession(100s, PatternType::RNDP, 100.0f, 67.0f, 0.0f, 4, true);
    engine.stop();

    // Start new session
    engine.startSession(200s, PatternType::RNDP, 100.0f, 67.0f, 0.0f, 4, true);
...
void test_TherapyEngine_getElapsedSeconds(void) {
    ...
    engine.startSession(7200s, PatternType::RNDP, 100.0f, 67.0f, 0.0f, 4, true);

    // Advance time by 5000ms (5 seconds)
    mockAdvanceMillis(5000);

    TEST_ASSERT_EQUAL(5s, engine.getElapsedSeconds());
}
...
void test_TherapyEngine_getRemainingSeconds(void) {
    ...
    TEST_ASSERT_EQUAL(70s, engine.getRemainingSeconds());
}

void test_TherapyEngine_elapsed_zero_when_not_running(void) {
    TherapyEngine engine;

    TEST_ASSERT_EQUAL(0s, engine.getElapsedSeconds());
}

void test_TherapyEngine_remaining_zero_when_not_running(void) {
    TherapyEngine engine;
    TEST_ASSERT_EQUAL(0s, engine.getRemainingSeconds().count());
}
...
void test_TherapyEngine_session_timeout(void) {
    ...
    engine.startSession(10s, PatternType::RNDP, 100.0f, 67.0f, 0.0f, 4, true);  // 10 second session
...
void test_TherapyEngine_pattern_type_sequential(void) {
    TherapyEngine engine;
    engine.setActivateCallback(mockActivateCallback);
    engine.startSession(7200s, PatternType::SEQUENTIAL, 100.0f, 67.0f, 0.0f, 4, true);
    engine.update();
...
void test_TherapyEngine_pattern_type_mirrored(void) {
    ...
    engine.startSession(7200s, PatternType::MIRRORED, 100.0f, 67.0f, 0.0f, 4, true);
    engine.update();
