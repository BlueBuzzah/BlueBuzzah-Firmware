...
class Pattern:
    ...
    def __init__(
        ...
        if len(left_sequence) != len(right_sequence):
            raise ValueError(
                "Sequence length mismatch: left={}, right={}".format(len(left_sequence), len(right_sequence))
            )

        # Validate timing matches sequence length
        if len(timing_ms) != len(left_sequence):
            raise ValueError(
                "Timing length {} doesn't match sequence length {}".format(len(timing_ms), len(left_sequence))
            )
